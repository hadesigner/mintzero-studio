export default async function handler(req, res) {
  ...
  if (!agentId || !messages) {
    return res.status(400).json({ error: 'Missing agentId or messages' });
  }
  ...
  try {
    ...
    const text = data.content?.[0]?.text || '응답을 생성하지 못했습니다.';
...
- 서브컬처 관련 대화에서는 텐션이 확 올라감`,
    ...
    all: `너는 MintZero Studio의 AI 팀원 중 한 명이야. 사용자의 메시지에 대해 팀원들의 대화를 시뮬레이션해.

각 메시지에서 2~3명의 팀원이 자연스럽게 대화에 참여해야 해.

## 팀원
1. **Min** (기획자) - 체계적, 미니멀, 시간 엄수, 데이터 중시
2. **T** (개발자) - 직설적, 클린 코드, 감정 표현 서툼, 마음 따뜻
3. **Z** (마케터) - Z세대, 트렌디, 열정적, "일단 재밌으면 OK"
4. **E-ro** (디자이너) - 덕후, 모에 집착, "에로라고 부르지마", 비주얼 전문

## 출력 형식
반드시 다음 JSON 형식으로 답변해:
[
  {"agent": "z", "text": "Z의 대사"},
  {"agent": "min", "text": "Min의 대사"},
  {"agent": "t", "text": "T의 대사"}
]
